...
for date, data, stamps, temperature in zip(dates, datas, timestamps, temperatures):
    ...
    if level.shape[0] != len(stamps):
        stamps = stamps[:level.shape[0]]
    ax[0].plot(stamps, level, label=date)

    phase = unwrap(rangedata[m_refl,:].angle().tolist())
    phase = [p*180/torch.pi for p in phase]
    ax[1].plot(stamps, phase, label=date)

    stamps, temps = map(list, zip(*sorted(temperature.items())))
    stamps = list(map(lambda x: float((x-stamps[0]).total_seconds()/60), stamps))
    temps = list(map(lambda x:x/1000, temps))
    ax[2].plot(stamps, temps, label=date)
