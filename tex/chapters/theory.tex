...
Here, $\sigma$ represents the \emph{radar cross section (RCS)} of the observed scene,
...
\end{align}
From this, the complex channel gain $\underline C(\vec r_S)$ can be defined:
\begin{align}
...
that each have their own radar cross section $\sigma_{l,k}, l \in \{0,1,...L-1\}$
\footnote{
...
Also, interference and electric noise may be present in each channel (see \cite{jankiraman}, ch. 4),
which we summarize as $n_k(t)$.
Thus, the overall IF-signal is:
\begin{align}
    y_k(t) & = \sum_{l=0}^{L-1} \underline C_k(\vec r_l; \sigma_{l,k}) e^{-j\dot\omega\tau_k(\vec r)t} + n_k(t)
\end{align}
